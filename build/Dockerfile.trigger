...
# Build
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -o bin/trigger github.com/knative-sample/tekton-serving/cmd/trigger/

FROM registry.cn-hangzhou.aliyuncs.com/knative-sample/alpine-sh:3.9
WORKDIR /app/
RUN mkdir -p /app/bin/
COPY --from=builder /go/src/github.com/knative-sample/tekton-serving/bin/trigger /app/bin/trigger
